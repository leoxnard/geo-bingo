...
export const calculateBingoCounter = (gridSize: number, board: string[], submissions: Submission[]): { count: number; players: string[] } => {
...
export const getGridLocations = (centerLat: number, centerLng: number, radiusMeters: number) => {
    ...
    for (const xFactor of offsetFactors) {
        for (const yFactor of offsetFactors) {
            ...
            if (offsetFactors.length > 2 && ((xFactor === offsetFactors[0] && yFactor === offsetFactors[0]) || (xFactor === offsetFactors[0] && yFactor === offsetFactors[offsetFactors.length - 1]) || (xFactor === offsetFactors[offsetFactors.length - 1] && yFactor === offsetFactors[0]) || (xFactor === offsetFactors[offsetFactors.length - 1] && yFactor === offsetFactors[offsetFactors.length - 1]))) {
                continue;
            }
