...
export default function DailyChallengeView({ date }: { date: string }) {
    ...
    if (phase === 'missing' || !challenge) {
        return (
            <div className="flex h-dvh flex-col items-center justify-center gap-4 bg-slate-950 px-6 text-center text-slate-300">
                <p className="text-lg font-medium">{t('daily.noChallenge')}</p>
                <Link href="/daily" className="press rounded-xl bg-gradient-to-r from-indigo-500 to-violet-500 px-5 py-2.5 font-bold uppercase text-white">
                    {t('daily.backToHub')}
                </Link>
            </div>
        );
            ...
            <main className="min-h-dvh bg-slate-950 px-4 py-8 text-white">
                <div className="mx-auto flex w-full max-w-2xl flex-col gap-6">
                    ...
                    <div className="rounded-2xl glass p-6 text-center">
                        ...
                        {(isAdmin || !user) && (
                            <button type="button" onClick={playAgain} className="mt-4 press rounded-xl bg-gradient-to-r from-indigo-500 to-violet-500 px-5 py-2.5 text-sm font-bold uppercase text-white">
    ...
    return (
        <div className="relative h-dvh w-full overflow-hidden bg-slate-950">
            ...
            <div className="pointer-events-none absolute inset-x-0 top-0 z-20 flex items-start justify-between gap-3 p-3 sm:p-4">
                ...
                <div className="text-center pointer-events-auto max-w-[55vw] rounded-2xl glass-dark px-4 py-2.5 shadow-xl">
                ...
                <div className="pointer-events-auto flex items-center gap-2">
                    <div className="rounded-2xl glass-dark px-4 py-2.5 font-mono text-xl font-bold tabular-nums text-indigo-300 shadow-xl" aria-live="off">
                        {/* tick forces re-render */}
                        <span className="sr-only">{tick}</span>
                        {formatDuration(elapsedMs)}
                    </div>
                    <button type="button" onClick={() => setConfirmingGiveUp(true)} className="h-full rounded-2xl glass-dark px-4 py-4 text-slate-300 shadow-xl hover:text-red-400" title={t('daily.giveUp')}>
                        <FaFlag />
                    </button>
                </div>
            </div>
            ...
            <div className="pointer-events-none absolute inset-x-0 bottom-0 z-20 flex flex-col items-center gap-2 p-4 sm:p-6">
                ...
                <button type="button" onClick={onFound} disabled={phase === 'verifying'} className="pointer-events-auto flex items-center gap-2 btn-sheen press rounded-full bg-gradient-to-r from-indigo-500 via-violet-500 to-fuchsia-500 px-8 py-3.5 text-base font-bold uppercase tracking-wide text-white shadow-2xl transition-all disabled:opacity-60">
                    {phase === 'verifying' ? (
                        <>
            ...
            {confirmingGiveUp && (
                <div className="absolute inset-0 z-30 flex items-center justify-center bg-black/60 p-4">
                    <div className="flex w-full max-w-sm flex-col gap-4 rounded-2xl glass p-6 text-center">
