...
export default function SafeImage({
  ...
  if (!currentSrc) {
    return (
      <div
        className={`animate-pulse bg-slate-700 flex items-center justify-center ${className || ""}`}
      >
  ...
  return (
    <img
      ...
      onError={() => {
        ...
        console.warn(
          `[SafeImage] Failed to load image due to potential rate limits. Retrying... (Attempt ${errorCount + 1})`,
        );
        if (errorCount < 5) {
