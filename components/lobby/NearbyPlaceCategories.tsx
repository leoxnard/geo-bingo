...
export const generateNearbyPlaceCategories = async (
  startPos: { lat: number; lng: number },
  radius: number,
  requiredCount: number,
  difficulty: "default" | "easy",
  language: string,
): Promise<BingoCategory[]> => {
  try {
    const googleApiKey = process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY;
    if (!googleApiKey) throw new Error("Google Maps API Key is missing!");

    const radiusMeters = radius * 100;
    const { points: gridPoints, subRadiusMeters } = getGridLocations(
      startPos.lat,
      startPos.lng,
      radiusMeters,
    );
    ...
    const fetchSingleTypeAtLocation = async (
      group: { name: string; types: string[] },
    ...
    allPlaces.forEach((place) => {
      ...
      const isTooClose = Array.from(uniquePlacesMap.values()).some(
        (existingPlace: { lat: number; lng: number }) => {
    ...
    if (shuffledPlaces.length < requiredCount) {
      throw new Error(
        `Not enough places found within the specified radius (${shuffledPlaces.length}/${requiredCount}).`,
      );
    }
    ...
    const cityCountry =
      shuffledPlaces[0].address?.split(",").slice(-2).join(",").trim() ||
      shuffledPlaces[0].address;

    const prompt = getPromptForNearbyPlaceCategories(
      cityCountry,
      uniquePlacesForLLM,
      requiredCount,
      difficulty,
    ...
    while (currentModelIndex < geminiModels.length) {
      try {
        aiResponse = await fetch(
          `https://generativelanguage.googleapis.com/v1beta/models/${geminiModels[currentModelIndex]}:generateContent?key=${geminiApiKey}`,
          {
            method: "POST",
        ...
        if (!aiResponse.ok) {
          ...
          throw new Error(
            `Gemini API error with model ${geminiModels[currentModelIndex]}: ${errorBody.error?.message || "Unknown AI error"}`,
          );
        }
    ...
    if (
      ...
      parsedCategories.length < requiredCount
    ) {
      throw new Error(
        `AI returned invalid format or fewer categories than required!`,
      );
    }

    const finalCategories: BingoCategory[] = parsedCategories
      ...
      .map((category) => {
        const enrichedPlaces = category.matchedPlaces
          ...
          .filter(
            (p: { name: string; lat: number; lng: number } | null) =>
              p !== null,
          );
    ...
    const errorMessage =
      error instanceof Error ? error.message : "Error fetching places.";
