-- =============================================================================
-- COMMUNITY PRESET CATEGORY TRANSLATIONS
-- =============================================================================
-- Each preset stores its category names translated into every app language, as
-- a jsonb map { "<locale>": ["name1", "name2", ...] } aligned to the categories
-- order. Produced at publish time by Gemini (source-language detection) + DeepL.
-- An imported game can then show categories in any single shared board language.
--
-- The create + update RPCs gain a trailing p_category_translations argument
-- (signatures change, so the previous versions are dropped and recreated).
-- =============================================================================
...
-- CREATE --------------------------------------------------------------------
DROP FUNCTION IF EXISTS public.create_community_preset(text, text, text, jsonb, jsonb, text, int, text, text, int, jsonb, text);
...
BEGIN
    ...
    INSERT INTO community_presets (
        ...
        starting_point, category_count, recommended_time, difficulty, game_mode, grid_size, settings, icon, category_translations
    )
    VALUES (
        caller,
...
-- UPDATE --------------------------------------------------------------------
DROP FUNCTION IF EXISTS public.update_community_preset(uuid, text, text, jsonb, jsonb, text, int, text, text, int, jsonb, text);
