...
export const ToggleButton = ({
  ...
  >
    ...
    >
      ...
      {/* Labels */}
      <div
        className={`relative z-10 flex-1 py-2 text-sm font-semibold transition-colors duration-200 ${active === "left" ? "text-white" : "text-slate-400"}`}
      >
        {labelLeft}
      </div>
      <div
        className={`relative z-10 flex-1 py-2 text-sm font-semibold transition-colors duration-200 ${active === "right" ? "text-white" : "text-slate-400"}`}
      >
        {labelRight}
      </div>
    </button>
...
export const MultiToggleButton = <T extends string | number>({
  ...
  const ratios =
    sizeRatios && sizeRatios.length === options.length
      ? sizeRatios
      : options.map(() => 1);

  const totalRatioSum = ratios.reduce((acc, val) => acc + val, 0);
  const currentRatio = ratios[safeActiveIndex];
  const prevRatiosSum = ratios
    .slice(0, safeActiveIndex)
      ...
      >
        ...
        {options.map((option, index) => {
          ...
          const isOptionDisabled =
            disabled ||
            (allowedValues && !allowedValues.includes(option.value));
          return (
            <button
              key={option.value}
              onClick={() => {
