...
export default function LobbyCategories({
  ...
  const [localGenerationNumber, setLocalGenerationNumber] =
    useState(generationNumber);
  ...
  const handleAIGeneration = async () => {
    ...
    try {
      const requiredCount =
        gameMode === "bingo" ? gridSize * gridSize : localGenerationNumber;
      const aiCategories = await generateAICategories(
        customPrompt,
        requiredCount,
        language,
      );

      const newCategories = aiCategories.map(
      ...
      toast.success(
        `Generated ${newCategories.length} AI categories successfully!`,
      );
    } catch (error) {
  ...
  const getAvailableWords = async () => {
    ...
    if (wordSource === "balanced") {
      ...
      allWords =
        language === "german" ? categoriesSimpleDe : categoriesSimpleEn;
      ...
      if (wordSource === "geo_all") {
        allWords = geoDb.map(
          (item: { term: string; category: string }) => item.term,
        );
      } else {
        const category = wordSource.replace("geo_", "");
        allWords = geoDb
          .filter(
            (item: { term: string; category: string }) =>
    ...
    if (trimmedCat !== "" && !isHost) {
      if (
        localCategories.some(
          (c) => (c ?? "").toLowerCase() === trimmedCat.toLowerCase(),
        )
      ) {
      ...
      if (
        currentSuggestions.some(
          (c: string) => c.toLowerCase() === trimmedCat.toLowerCase(),
        )
      ) {
      ...
      await supabase
        .from("games")
        .update({ suggested_categories: updatedSuggestions })
        .eq("id", gameId);
      setNewCategory("");
  ...
  const rejectSuggestion = async (cat: string) => {
    ...
    await supabase
      .from("games")
      .update({ suggested_categories: updatedSug })
      .eq("id", gameId);
  };
  ...
  return (
    <div className="bg-slate-800 p-6 rounded-xl flex-1 border border-slate-700 h-fit">
      {categoriesGenerated === false && (
        <>
          <MultiToggleButton
            ...
            disabled={!isHost}
            allowedValues={
              startingPoint === "open-world" ? ["manual", "ai"] : undefined
            }
            isHost={isHost}
            position="top"
          ...
          {categorySource === "ai" && (
            <div className="py-3 border-t border-slate-700">
              ...
              <textarea
                ...
                placeholder={
                  isHost
                    ? "Enter a theme like 'car brands', 'construction elements', or 'vintage signs'. Leave empty for random categories."
                    : "Waiting for host to generate categories..."
                }
      ...
      {(categorySource === "manual" ||
        (categorySource === "ai" && categoriesGenerated)) && (
        <>
          <h3
            className={`text-xl font-bold mb-4 text-slate-300 flex justify-between items-center transition-all ${categorySource === "manual" ? "pt-4 border-t border-slate-700" : ""}`}
          >
            <span>Categories</span>
            <div className="flex items-center">
              <span
                className={`text-sm font-normal ${localCategories.length === 0 || (gameMode === "bingo" && localCategories.length < gridSize * gridSize) ? "text-red-400" : "text-slate-400"} bg-slate-900 px-3 py-1 rounded-full`}
              >
                {gameMode === "bingo"
              ...
              {isHost && (
                <button
                  ...
                  className="text-xs font-bold text-slate-400 bg-slate-800 hover:bg-slate-700 hover:text-white px-3 py-1 rounded-full ml-2 transition-colors"
                >
                  Clear
                </button>
              )}
          ...
          gameMode === "bingo" ? (
            <div className={`grid gap-3 mb-6 bingo-grid-${gridSize}`}>
              {Array.from({
                ...
                return (
                  ...
                  >
                    <span
                      className={`italic w-full ${cat ? "text-white font-medium" : "text-slate-500"}`}
                    >
                      {cat || "Empty"}
                    </span>
                  </div>
            ...
            <ul className="mb-6 space-y-2">
              {categories.map((cat, i) => (
                <li
                  key={`view-list-${i}`}
                  className="bg-slate-700 rounded-lg flex items-center border border-slate-600 italic shadow-sm overflow-hidden h-[42px]"
                >
            ...
            <div className="text-center text-slate-500 italic py-6 border-2 border-dashed border-slate-700 rounded-lg">
              No categories yet. Suggest some categories or wait for the Host to
              generate them.
            </div>
          )}
          ...
          <div className="p-4 bg-slate-800/80 rounded-xl border border-dashed border-indigo-500/50 mt-6">
            ...
            <ul className="space-y-2">
              {suggestedCategories.length === 0 ? (
                <li className="text-slate-500 italic py-2">
                  No suggestions yet
                </li>
              ) : (
                suggestedCategories.map((cat, i) => (
                  <li
                    key={i}
                    className="bg-slate-700 rounded-lg flex justify-between items-center border border-slate-600 italic shadow-sm overflow-hidden p-1 h-[42px]"
                  >
                    <span className="break-words py-2 px-3 flex items-center text-white">
                      {cat}
                    </span>
                    {isHost && (
