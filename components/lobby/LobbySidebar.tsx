...
export default function LobbySidebar(props: LobbySidebarProps) {
  ...
  const maxTeam =
    props.players.length > 0
      ? Math.max(...props.players.map((p) => p.team || 0))
      : 0;
  const displayTeamCount =
    props.teamMode === "teams" ? Math.max(teamCount, maxTeam + 1) : teamCount;
  ...
  const saveSelfName = async () => {
    const currentName =
      props.players.find((p) => p.id === props.playerId)?.name || "";
    ...
    props.setPlayers((prev) =>
      prev.map((p) => (p.id === props.playerId ? { ...p, name: nextName } : p)),
    );
  ...
  const handleUpdatePlayerTeam = async (
    targetPlayerId: string,
    teamIndex: number,
  ) => {
    props.setPlayers((prev) =>
      prev.map((p) =>
        p.id === targetPlayerId ? { ...p, team: teamIndex } : p,
      ),
    );
    const { error } = await props.supabase
      .from("players")
      .update({ team: teamIndex })
  ...
  const handleRemoveTeam = async (teamIndexToRemove: number) => {
    ...
    const updates = updatedPlayers
      .filter(
        (p) =>
          (p.team || 0) >= teamIndexToRemove ||
          props.players.find((old) => old.id === p.id)?.team ===
            teamIndexToRemove,
      )
      .map((p) =>
        props.supabase.from("players").update({ team: p.team }).eq("id", p.id),
  ...
  const renderPlayerItem = (p: Player) => (
    ...
    >
      <div className="flex items-center gap-3">
        <div
          className={`min-w-[8px] h-2 rounded-full animate-pulse ${props.onlinePlayers.includes(p.id) ? "bg-green-500" : "bg-orange-500"}`}
        />
        <div className="flex-1 min-w-0 flex items-center gap-2">
          {p.id === props.playerId && isEditingSelfName ? (
            ...
            <span
              className={`flex-1 truncate ${p.id === props.playerId ? "text-green-400" : "text-white"}`}
            >
      ...
      {props.isHost && p.id !== props.playerId && (
        <div className="flex gap-2 w-full mt-1 border-t border-slate-800 pt-2">
          <button
            type="button"
            onClick={() => props.makeHost(p.id)}
            className="text-[10px] flex-[2] bg-indigo-900/50 text-indigo-400 hover:bg-indigo-600 hover:text-white py-1 rounded"
          >
            Make Host
          </button>
          <button
            ...
            className="text-[10px] flex-1 bg-orange-900/50 text-orange-400 hover:bg-orange-600 hover:text-white py-1 rounded"
          >
            Kick
          </button>
          <button
            type="button"
            onClick={() => props.banPlayer(p.id)}
            className="text-[10px] flex-1 bg-red-900/50 text-red-400 hover:bg-red-600 hover:text-white py-1 rounded"
          >
            Ban
          </button>
        </div>
  ...
  return (
    <div className="flex flex-col gap-6 w-full lg:w-80">
      ...
      <div className="bg-slate-800 p-6 rounded-xl border border-slate-700 h-fit">
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2 bg-slate-900 border border-slate-700 p-2 rounded-lg">
            <span className="text-sm font-bold text-slate-400 w-12 tracking-widest">
              ID:
            </span>
            <span className="flex-1 font-mono text-slate-300 text-lg truncate">
              {props.gameId}
            </span>
            <button
              type="button"
      ...
      <div className="bg-slate-800 p-6 rounded-xl border border-slate-700 h-fit">
          ...
          {props.teamMode === "teams" && (
            <div className="flex gap-2">
              ...
              <button
                onClick={() =>
                  setTeamCount(
                    Math.min(displayTeamCount + 1, darkTeamColors.length),
                  )
                }
        ...
        {/* Free For All Mode */}
        {props.teamMode === "ffa" && (
          <ul className="space-y-3">{props.players.map(renderPlayerItem)}</ul>
        )}
        ...
        {props.teamMode === "teams" && (
          <div className="space-y-4">
            {Array.from({ length: displayTeamCount }).map((_, teamIndex) => {
              const teamPlayers = props.players.filter(
                (p) => (p.team || 0) === teamIndex,
              );
              const colorClass =
                darkTeamColors[teamIndex % darkTeamColors.length];
        ...
        <button
          ...
          className="w-full py-3 rounded-xl font-bold mt-3 border border-slate-600 text-slate-300 hover:bg-slate-700 transition-colors"
        >
          Leave Lobby
        </button>
      </div>
      <div className="bg-slate-800 p-6 rounded-xl border border-slate-700 h-fit">
        ...
        <div className="flex flex-col gap-5">
          ...
          <ToggleSwitch
            ...
            disabled={!props.isHost}
            onChange={(checked) =>
              props.updateGameModeInfo({ hide_map_symbols: checked })
            }
          />
          {/* Toggle 2: Hide Mini Map */}
          <ToggleSwitch
            label="Hide Mini Map"
            checked={props.hideMiniMap}
            disabled={!props.isHost}
            onChange={(checked) =>
              props.updateGameModeInfo({ hide_minimap: checked })
            }
          />
          {!props.isHost && (
