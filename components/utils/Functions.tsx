...
export const calculateBingoCounter = (
  ...
  if (!board || board.length === 0 || gridSize < 2)
  ...
  for (let j = 0; j < gridSize; j++) {
    ...
    if (columnComplete) {
      bingoCount++;
      for (let i = 0; i < gridSize; i++)
        contributingPlayers.add(grid[i][j]!.player_id);
    }
  }
  ...
  if (diag1Complete) {
    bingoCount++;
    for (let i = 0; i < gridSize; i++)
      contributingPlayers.add(grid[i][i]!.player_id);
  }
  ...
  if (diag2Complete) {
    bingoCount++;
    for (let i = 0; i < gridSize; i++)
      contributingPlayers.add(grid[i][gridSize - 1 - i]!.player_id);
  }
...
export const getGridLocations = (
  ...
  for (const xFactor of offsetFactors) {
    for (const yFactor of offsetFactors) {
      ...
      if (
        offsetFactors.length > 2 &&
        ((xFactor === offsetFactors[0] && yFactor === offsetFactors[0]) ||
          (xFactor === offsetFactors[0] &&
            yFactor === offsetFactors[offsetFactors.length - 1]) ||
          (xFactor === offsetFactors[offsetFactors.length - 1] &&
            yFactor === offsetFactors[0]) ||
          (xFactor === offsetFactors[offsetFactors.length - 1] &&
            yFactor === offsetFactors[offsetFactors.length - 1]))
      ) {
        continue;
