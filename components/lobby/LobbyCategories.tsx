...
export default function LobbyCategories({
    ...
    const acceptSuggestion = async (cat: string) => {
        ...
        // Optimistic UI für das Annehmen der Suggestions anwenden:
        isPendingSyncRef.current = true;
        ...
        <div className="bg-slate-800 p-6 rounded-xl flex-1 border border-slate-700 h-fit">
            <MultiToggleButton
                ...
                options={[
                    ...
                    { value: 'nearbyPlaces', label: 'Nearby Places (AI)' },
                    { value: 'nearbyStreetView', label: 'Nearby Street View Features (AI)' }
                ]}
                activeValue={categorySource}
                onChange={(val) => updateGameModeInfo({ category_source: val })}
            ...
            {categorySource === 'manual' && (
                <>
                    <h3 className="text-xl font-bold mb-4 text-slate-300 flex justify-between items-center transition-all pt-4 border-t border-slate-700">
