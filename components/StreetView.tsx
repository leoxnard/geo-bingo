...
export default function StreetView({
  ...
  const [isMobileLandscape, setIsMobileLandscape] = useState(() => {
    ...
    return window.matchMedia("(max-width: 932px) and (orientation: landscape)")
      .matches;
  });

  const [panoInstance, setPanoInstance] =
    useState<google.maps.StreetViewPanorama | null>(null);
  const [minimapInstance, setMinimapInstance] =
    useState<google.maps.Map | null>(null);
  const [mainMapInstance, setMainMapInstance] =
    useState<google.maps.Map | null>(null);
  ...
  const myTeam = useMemo(
    () => players.find((p) => p.id === playerId)?.team ?? -1,
    [players, playerId],
  );
  const teamIds = useMemo(
    () =>
      teamMode === "teams"
        ...
        : [playerId],
    [teamMode, players, myTeam, playerId],
  );

  const mySubmissions = useMemo(
    () => allSubmissions.filter((s) => teamIds.includes(s.player_id)),
    [allSubmissions, teamIds],
  );
    ...
    if (minimapInstance && panoInstance && !hideMiniMap) {
      ...
      const renderPathSegments = () => {
        ...
        let currentSegment: { lat: number; lng: number }[] = [
          { lat: pathRef.current[0].lat, lng: pathRef.current[0].lng },
        ];
      ...
      const positionListener = panoInstance.addListener(
        "position_changed",
        () => {
  ...
  const onLoad = useCallback(
    (pano: google.maps.StreetViewPanorama) => {
      ...
      if (startingPoint !== "open-world") {
        ...
        const googleStartingPoint = new google.maps.LatLng(
          parsedStart.lat,
          parsedStart.lng,
        );
        ...
        lastValidPositionRef.current = new google.maps.LatLng(
          parsedStart.lat,
          parsedStart.lng,
        );
      }
      ...
      pano.addListener("position_changed", () => {
        ...
        if (isValidLocation) {
          ...
          toast(
            "You've reached the edge of the allowed area or entered a forbidden zone!",
          );
  ...
  const handleSubmit = async (targetCategory: string) => {
    ...
    if (gameMode === "bingo" && endCondition === "first_bingo") {
      const bingos = calculateBingoCounter(
        gridSize,
        myBoard,
        updatedMySubmissions,
      );

      if (bingos.count > 0) {
        const winnerNames = players
          .filter((p) => bingos.players.includes(p.id))
          .map((p) => p.name);
        let winnerNamesString;
        if (winnerNames.length > 2) {
          winnerNamesString = [
            winnerNames.slice(0, -1).join(", "),
            winnerNames.slice(-1)[0],
          ].join(" and ");
    ...
    if (exclusiveMode && !existingSub) {
      ...
      if (data && data.success === false && data.error === "ALREADY_CLAIMED") {
        ...
        setAllSubmissions((prev) =>
          prev.map((s) => (s.id === tempId ? data.data : s)),
        );
      }
    } else {
  ...
  const handleBingoTileClick = (cat: string) => {
    const isBlocked =
      exclusiveMode &&
      !mySubmissions.find((s) => s.category === cat) &&
      otherSubmissions.some((s) => s.category === cat);
      ...
      try {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          if (parsed[0].lat !== undefined && parsed[0].id === undefined) {
            parsedBoundaries = [
              { id: "legacy", type: "allow", points: parsed },
            ];
          } else {
            parsedBoundaries = parsed;
          }

          const allowBoundaries = parsedBoundaries.filter(
            (b) => b.type !== "forbid",
          );
          const boundariesToCalculate =
            allowBoundaries.length > 0 ? allowBoundaries : parsedBoundaries;

          const allPoints = boundariesToCalculate.flatMap(
            (b) => b.points || [],
          );
          ...
          if (allPoints.length >= 3) {
            ...
            const calculatedZoom =
              maxDiff > 0
                ? Math.floor(Math.log2(360 / maxDiff)) + 1
                : initialWorldZoom;
  ...
  if (!isLoaded)
    return (
      <div className="h-screen flex items-center justify-center text-indigo-400">
        Loading Maps...
      </div>
    );
  ...
  return (
    <div className="min-h-screen p-4 bg-slate-900">
      ...
      <div className="w-full max-w-[95%] xl:max-w-[90vw] mx-auto">
        {playerId && (
          <div
            className={`flex gap-6 ${isMobileLandscape ? "flex-row h-[calc(100dvh-7rem)] min-h-0" : "flex-col lg:flex-row h-[calc(100vh-8rem)] min-h-[600px]"}`}
          >
            {/* Left: Map */}
            <div
              ref={containerRef}
              className={`${isMobileLandscape ? "basis-[58%] min-h-0 h-full" : "flex-1 min-h-[400px] h-full"} border-4 border-slate-700 rounded-2xl overflow-hidden shadow-2xl relative bg-slate-800 absolute-safari-fix`}
            >
              <GoogleMap
                key={gameId}
                  ...
                  (boundary, index) =>
                    ...
                    boundary.points.length >= 3 && (
                      <Polygon
                        ...
                        options={{
                          fillColor:
                            boundary.type === "allow" ? "#008000" : "#ff0000",
                          fillOpacity: 0.1,
                          strokeColor:
                            boundary.type === "allow" ? "#008000" : "#ff0000",
                          strokeOpacity: 0.6,
            ...
            {/* Right: Checklist */}
            <div
              className={`${isMobileLandscape ? "basis-[42%] max-w-[42%]" : `w-full ${getSidebarWidthClass()}`} flex flex-col gap-4 bg-slate-800 p-6 rounded-2xl shadow-xl h-full border border-slate-700 overflow-y-auto transition-all`}
            >
              ...
              {gameMode === "list" ? (
                <ul className="flex flex-col gap-3 flex-1">
                  {myBoard.map((cat) => {
                    const foundSub = mySubmissions.find(
                      (s) => s.category === cat,
                    );
                    const isBlocked =
                      exclusiveMode &&
                    ...
                    const fov = foundSub?.zoom
                      ? 180 / Math.pow(2, foundSub.zoom)
                      : 90;
                        ...
                        <div className="relative z-10 flex flex-col gap-2">
                          <div className="flex justify-between items-center w-full">
                            <div className="flex items-center flex-1 pr-2 min-w-0">
                              <span
                                className={`truncate font-medium ${foundSub ? "text-white drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]" : isBlocked ? "text-red-400 line-through" : "text-white"}`}
                              >
                                {cat}
                              </span>
                              {hint && (
                                <div
                                  className="ml-2 relative group flex-shrink-0 cursor-help"
                                  onClick={(e) => e.stopPropagation()}
                                >
                                  <FaInfoCircle
                                    className={`transition-colors ${foundSub ? "text-white/70 hover:text-white" : "text-slate-400 hover:text-white"}`}
                                    size={14}
                                  />
                            ...
                            <span
                              className={`text-xs font-bold uppercase whitespace-nowrap ${foundSub ? "text-green-400 drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]" : isBlocked ? "text-red-500" : "text-slate-500"}`}
                            >
                              {foundSub
                                ? "Found"
                                : isBlocked
                ...
                <div
                  className={`grid gap-2 flex-1 auto-rows-fr bingo-grid-${gridSize}`}
                >
                  {myBoard.map((cat) => {
                    const foundSub = mySubmissions.find(
                      (s) => s.category === cat,
                    ...
                    return (
                      <div
                        key={cat}
                        title={
                          isBlocked ? "Claimed by another team" : undefined
                        }
                      ...
                      >
                        ...
                        {hint && (
                          ...
                          >
                            <FaInfoCircle
                              className={`transition-colors text-[11px] sm:text-sm drop-shadow-md ${foundSub ? "text-white/70 hover:text-white" : "text-slate-400/70 hover:text-white"}`}
                            />
                        ...
                        <span
                          className={`relative z-10 ${getSidebarTextSizeClass()} font-bold leading-tight line-clamp-3 [hyphens:auto] [word-break:break-word] mt-0 sm:mt-1 ${foundSub ? "drop-shadow-[0_5px_5px_rgba(0,0,0,0.8)]" : isBlocked ? "text-red-400 line-through" : "text-white"}`}
                        >
                          {cat}
                        </span>
                          ...
                          {!foundSub ? (
                            ...
                            >
                              {submittingCategory === cat ? (
                                ...
                                <FaCamera className="h-[60%] w-auto" />
                              )}
                            </button>
                          ) : (
                            <>
                              <button
                                ...
                                disabled={
                                  submittingCategory === cat || !inStreetView
                                }
