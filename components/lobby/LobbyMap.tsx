...
export default function LobbyMap({
  ...
  const [boundaryPresetsData, setBoundaryPresetsData] = useState<
    Record<string, any[]>
  >({});
  ...
  const updateSearchAndSelection = (value: string) => {
    ...
    const firstValidGroup = groupNames.find((g) =>
      groupedPresets[g]?.some((item) =>
        item.replace(/_/g, " ").toLowerCase().includes(normalized),
      ),
    );
  ...
  const activeBoundaryId = useMemo(() => {
    ...
    if (
      selectedBoundaryId &&
      draftBoundaries.some((b) => b.id === selectedBoundaryId)
    ) {
  ...
  useEffect(() => {
    ...
    const visibleListener = google.maps.event.addListener(
      sv,
      "visible_changed",
      () => {
        if (sv.getVisible()) {
  ...
  const handleMapClick = (e: google.maps.MapMouseEvent) => {
    ...
    if (newBoundaries.length === 0) {
      ...
      const targetId =
        activeBoundaryId || newBoundaries[newBoundaries.length - 1].id;
  ...
  const handleDrop = (dropIndex: number) => {
    ...
    newGroups.forEach((group) => {
      const itemsInGroup = draftBoundaries.filter(
        (b) => (b.groupId || b.id) === group.key,
      );
    ...
    if (e.key === "ArrowDown") {
      ...
      if (current?.type === "item") {
        const parentIdx = visibleItems.findIndex(
          (i) => i.type === "group" && i.value === current.parentGroup,
        );
      ...
      if (current?.type === "item") {
        ...
        else if (filteredGroups[current.value].length > 0)
          setExpandedGroup(current.value);
      }
  ...
  const handleRemoveGroup = (key: string) => {
    setSelectedPreset("");
    const newBoundaries = draftBoundaries.filter(
      (b) => b.id !== key && b.groupId !== key,
    );
    updateGameModeInfo({ gameBoundary: JSON.stringify(newBoundaries) });
  ...
  const handleToggleGroupType = (key: string) => {
    setSelectedPreset("");
    const groupItem = draftBoundaries.find(
      (b) => b.id === key || b.groupId === key,
    );
  ...
  return (
    <div className="bg-slate-800 p-6 rounded-xl flex-1 border border-slate-700 h-fit">
      ...
      <p className="text-xs text-slate-400 mb-4">
        Left-click the map to draw movement boundaries. If multiple areas are
        defined, the priority defines the order of precedence. Drop the Pegman
        to set a custom starting point, or select a recommended city marker.
        Selecting a starting point will automatically disable exiting street
        view ingame.
      </p>
        ...
        <div className="h-[400px] min-h-[400px] w-full rounded-lg overflow-hidden border border-slate-700 relative bg-slate-800/50 flex flex-col items-center justify-center">
          {!isLoaded || presetsLoading ? (
            <div className="text-slate-400">
              Loading map configuration and presets...
            </div>
          ) : (
              ...
              >
                {actualStart.startsWith("{") && (
                  <MarkerF
                    ...
                    onMouseOver={() =>
                      setHoveredLocation(JSON.parse(actualStart))
                    }
                    ...
                    {boundary.points.length > 0 && (
                      <PolygonF
                        ...
                        options={{
                          ...
                          strokeOpacity: 0.6,
                          strokeWeight:
                            activeBoundaryId === boundary.id ? 4 : 2,
                          clickable: false,
                        }}
                      />
                    )}
                ...
                <div className="absolute left-0 top-full w-full pt-1 z-[100]">
                  <div className="bg-slate-800 border border-slate-700 rounded-lg shadow-xl py-1 max-h-64 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                    {visibleItems.length > 0 ? (
                      ...
                      <div className="px-4 py-2 text-slate-500 text-sm italic">
