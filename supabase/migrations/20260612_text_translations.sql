-- =============================================================================
-- PRESET TITLE/DESCRIPTION TRANSLATIONS  +  GAME CATEGORY TRANSLATIONS
-- =============================================================================
--  * community_presets.title_translations / description_translations: the preset
--    name + description translated into every app locale (DeepL at publish), so
--    browse cards can render in the viewer's language.
--  * games.category_translations: a copy of the preset's per-locale category map,
--    carried onto an imported game so the lobby's "Category Language" switch can
--    reuse those aligned translations (and fall back to DeepL otherwise).
--
-- The create/update preset RPCs gain two trailing jsonb args (signatures change,
-- so the previous versions are dropped and recreated).
-- =============================================================================
...
-- CREATE --------------------------------------------------------------------
DROP FUNCTION IF EXISTS public.create_community_preset(text, text, text, jsonb, jsonb, text, int, text, text, int, jsonb, text, jsonb);
...
-- UPDATE --------------------------------------------------------------------
DROP FUNCTION IF EXISTS public.update_community_preset(uuid, text, text, jsonb, jsonb, text, int, text, text, int, jsonb, text, jsonb);
