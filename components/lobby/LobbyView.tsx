...
export default function LobbyView(props: LobbyViewProps) {
  ...
  const handleStartGame = async () => {
    ...
    try {
      ...
      if (error)
        console.error("Error clearing player paths on game start:", error);
    } catch (err) {
    ...
    const neededCount =
      props.gameMode === "bingo"
        ? props.gridSize * props.gridSize
        : props.generationNumber;

    if (
      (props.categorySource === "nearbyPlaces" ||
        props.categorySource === "nearbyStreetView") &&
      props.startingPoint !== "open-world" &&
      startPos
    ) {
        ...
        const generatedCategoryNames = await toast.promise(
          (async () => {
            ...
            if (props.categorySource === "nearbyStreetView") {
              complexResult = await generateNearbyStreetViewCategories(
                startPos,
                props.generationRadius,
                neededCount,
                props.difficulty,
                props.language,
              );
            } else {
              complexResult = await generateNearbyPlaceCategories(
                startPos,
                props.generationRadius,
                neededCount,
                props.difficulty,
            ...
            const simpleCategoryNames = complexResult.map(
          ...
          })(),
          {
            loading:
              props.categorySource === "nearbyStreetView"
                ? "Analysze Street-View-Panoramas..."
                : "Generating...",
  ...
  return (
    <div className="min-h-screen flex flex-col items-center p-10 bg-slate-900 text-white relative">
      <div className="flex flex-col sm:flex-row items-center justify-center gap-6 mb-12 hidden sm:flex">
        ...
        />
        <h1 className="text-6xl font-bold text-indigo-400 tracking-tighter">
          GEO BINGO
        </h1>
      </div>
