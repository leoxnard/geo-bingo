  ...
  useEffect(() => {
    const fetchResults = async () => {
      const { data: game } = await supabase
        .from("games")
        .select("game_mode, grid_size, end_condition")
        .eq("id", gameId)
        .single();
      ...
      if (players && submissions) {
        ...
        const playerStats = entities.map((entity) => {
          ...
          const entitySubs = submissions.filter((s) =>
            memberIds.includes(s.player_id),
          );
          ...
          if (
            fetchedGameMode === "bingo" &&
            entity.bingo_board &&
            entity.bingo_board.length >= gridSize * gridSize
          ) {
            ...
            if (isDiag2) {
              for (let i = 0; i < gridSize; i++)
                gridStatus[i * gridSize + (gridSize - 1 - i)] = 3;
            }
        ...
        playerStats.forEach((p, i) => {
          if (i > 0) {
            ...
            const isBingoTie =
              game?.end_condition === "first_bingo"
                ? p.bingos === prev.bingos
                : true;
      ...
      <div className="w-full max-w-6xl mx-auto flex flex-col items-center text-white">
        ...
        <div className="w-full flex justify-between items-center mb-4 mt-4">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-4xl font-black uppercase tracking-widest text-indigo-400">
              Results
            </h1>
          </div>
          {isHost ? (
            <button
              ...
              onClick={async () => {
                ...
                const { error } = await supabase
                  .from("games")
                  .update({ status: "lobby", ready_players: [] })
                  .eq("id", gameId);
        ...
        <div className="flex items-end justify-center gap-4 md:gap-8 h-65 mt-16 w-full">
          ...
          {rank2.length > 0 && (
            <div className="flex flex-col items-center w-32 md:w-40 animate-[slideUp_1s_ease-out]">
              ...
              <span className="text-slate-400 mb-4 font-bold bg-slate-800 px-4 py-1 rounded-full">
                {renderScoreBadge(rank2[0])}
              </span>
            ...
            <div className="flex flex-col items-center w-40 md:w-48 animate-[slideUp_0.8s_ease-out]">
              <div className="flex flex-col items-center mb-2 w-full">
                {rank1.map((p) => (
                  <span
                    key={p.id}
                    className="text-2xl md:text-3xl font-black text-yellow-400 w-full text-center"
                    title={p.name}
                  >
                    {p.name}
                  </span>
              ...
              <span className="text-yellow-200 mb-4 font-bold bg-yellow-900/50 px-5 py-1 rounded-full">
                {renderScoreBadge(rank1[0])}
              </span>
              <div className="w-full bg-yellow-400 h-48 rounded-t-2xl flex justify-center items-start pt-6 shadow-[0_0_60px_rgba(250,204,21,0.4)] relative overflow-hidden">
                ...
                <span className="text-6xl font-black text-yellow-600 relative z-10">
                  1
                </span>
              </div>
            </div>
          ...
          {rank3.length > 0 && (
            <div className="flex flex-col items-center w-32 md:w-40 animate-[slideUp_1.2s_ease-out]">
              ...
              <span className="text-amber-600 mb-4 font-bold bg-amber-900/30 px-4 py-1 rounded-full">
                {renderScoreBadge(rank3[0])}
              </span>
        ...
        <div className="w-full bg-slate-800 rounded-3xl border border-slate-700 p-8 md:p-10 shadow-2xl">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {stats.map((player) => (
              <div
                key={player.id}
                className="bg-slate-900 p-6 rounded-2xl border border-slate-700 flex flex-col gap-4"
              >
                {/* Header: Rank & Name */}
                ...
                {/* Stats Grid */}
                <div
                  className={`grid ${gameMode === "bingo" ? "grid-cols-3" : "grid-cols-2"} gap-3 mt-2`}
                >
                  {/* Bingos */}
                  {gameMode === "bingo" && (
                  ...
                  {gameMode === "bingo" ? (
                    <div className="relative group bg-slate-800 p-3 rounded-xl flex flex-col items-center justify-center">
                      <span className="text-[10px] text-slate-400 uppercase font-bold mb-2 text-center">
                        Bingo-Board
                      </span>
                      <div
                      ...
                      >
                        {player.gridStatus?.map(
                          (status: number, idx: number) => (
                            <div
                              key={idx}
                              className={`w-2 h-2 rounded-sm ${
                                status === 3
                                  ? "bg-yellow-400 shadow-[0_0_8px_rgba(250,204,21,0.6)] z-10"
                                  : status === 1
                        ...
                        >
                          {player.bingoBoard?.map(
                            (word: string, idx: number) => {
                              ...
                              if (status === 3)
                                styleClass =
                                  "bg-yellow-900/40 text-yellow-400 border-yellow-500/50 shadow-[0_0_10px_rgba(250,204,21,0.2)] font-bold";
                              else if (status === 1)
                                styleClass =
                                  "bg-green-900/30 text-green-400 border-green-500/40";
                              else if (status === 2)
                                styleClass =
                                  "bg-red-900/20 text-red-500 border-red-500/30 line-through opacity-60";
                    ...
                    <div className="bg-slate-800 p-3 rounded-xl flex flex-col items-center h-full">
                      <span className="text-[10px] text-slate-400 uppercase font-bold mb-1 text-center">
                        Approve-Rate
                      </span>
                      <span
                        className={`text-xl font-medium ${
                          player.communityApproval >= 75
                            ? "text-green-400"
                            : player.communityApproval >= 50
                              ? "text-yellow-400"
                              : "text-red-400"
                        }`}
                      >
                  ...
                  <div
                    className={`bg-slate-800 p-3 rounded-xl flex flex-col ${gameMode === "bingo" ? "col-span-3" : "col-span-2"}`}
                  >
