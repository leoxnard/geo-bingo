...
export default function Home() {
  ...
  const handleSaveName = () => {
    const finalName =
      playerName.trim() ||
      `${showBadNames ? badAdjectives[Math.floor(Math.random() * badAdjectives.length)] : adjectives[Math.floor(Math.random() * adjectives.length)]}${animals[Math.floor(Math.random() * animals.length)]}`;
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center p-24 bg-slate-900 text-white">
      <div className="flex flex-col sm:flex-row items-center justify-center gap-6 mb-12 hover">
        ...
        <h1 className="text-6xl font-bold text-indigo-400 tracking-tighter hidden sm:block">
          GEO BINGO
        </h1>
      </div>
      ...
      <div className="bg-slate-800 p-8 rounded-2xl shadow-2xl border border-slate-700 w-full max-w-md flex flex-col gap-6">
        ...
        <div>
          <button
            type="button"
            className="text-sm text-slate-400 font-bold uppercase mb-2 block"
            onClick={() => setShowBadNames(!showBadNames)}
          >
        ...
        <div className="text-center text-slate-500 text-sm font-medium uppercase tracking-widest">
          or
        </div>
