...
export function VotingView({
  ...
  const currentBoard = useMemo(() => {
    ...
    return gameCategories.slice(
      0,
      gameMode === "list" ? gameCategories.length : gridSize * gridSize,
    );
  ...
  const votingStats = useMemo(() => {
    ...
    if (activeSubLatest) {
      ...
      const actualVotes = Object.keys(votesMap).filter(
        (k) => k !== "host_continued",
      );
      cast = actualVotes.length;

      const eligibleVoters = playersWithPaths.filter((p) =>
        teamMode === "teams"
  ...
  useEffect(() => {
    const fetchData = async () => {
      const { data: gData } = await supabase
        .from("games")
        .select(
          "categories, grid_size, game_mode, category_details, generation_radius, starting_point, category_source",
        )
        .eq("id", gameId)
        .single();
      ...
      if (gData) {
        ...
        setStartingPoint(
          gData.starting_point !== "open-world" &&
            gData.category_source !== "manual"
            ? JSON.parse(gData.starting_point)
            : null,
        );
      }
      ...
      const { data: pData } = await supabase
        .from("players")
        .select("id, name, team, path, bingo_board")
        .eq("game_id", gameId);
      if (pData) {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          setSubmissions((prev) =>
            prev.map((s) =>
              s.id === payload.new.id ? { ...s, votes: payload.new.votes } : s,
            ),
          );
        },
      )
  ...
  const pathData = useMemo(() => {
    ...
    const subsForPlayer = submissions.filter(
  ...
  useEffect(() => {
    ...
    const initMap = () => {
      if (pathData.rawPath.length > 1) {
        ...
        pathData.rawPath.forEach((p: { lat: number; lng: number }) =>
  ...
  useEffect(() => {
    ...
    const animate = (time: DOMHighResTimeStamp) => {
      ...
      let crossedSub = pathData.subProgressions.find(
        (sp) =>
          sp.progress <= progress && !shownSubIdsRef.current.has(sp.sub.id),
      );

      if (progress === 1 && !crossedSub) {
        const unshownSub = pathData.subProgressions.find(
          (sp) => !shownSubIdsRef.current.has(sp.sub.id),
        );
      ...
      if (progress <= 0) {
        ...
        let idx = 0;
        while (
          idx < pathData.dists.length - 2 &&
          pathData.dists[idx + 1] < targetDist
        ) {
          idx++;
        }
        ...
        const t =
          segmentDist === 0
            ? 0
            : (targetDist - pathData.dists[idx]) / segmentDist;
  ...
  const handleVote = async (sub: Submission, voteIsYes: boolean) => {
    ...
    setSubmissions((prev) =>
      prev.map((s) => (s.id === sub.id ? { ...s, votes: newVotes } : s)),
    );
  ...
  const activeCategoryMarkers = useMemo(() => {
    ...
    const activeCat = categoryDetails.find(
      (cat) => cat.categoryName === displaySub.category,
    );
        ...
        <MarkerF
          ...
          options={{
            ...
            label: {
              text: place.name,
              className:
                "bg-slate-900/90 text-white p-3 rounded-lg border border-indigo-500 text-[10px] font-bold mt-10 whitespace-nowrap shadow-lg",
              color: "#a4b3ff",
            },
          }}
          animation={
  ...
  const groupedFinalPlaces = useMemo(() => {
    ...
    const map = new Map<
      string,
      { lat: number; lng: number; categoryNames: string[] }
    >();
  ...
  if (isLineComplete) {
    ...
    const currentPlayerSubmissions = submissions.filter(
      (s) => s.player_id === currentPlayer?.id,
    );
  ...
  const panoramaKey = selectedSubmission?.id
    ? `submission-${selectedSubmission.id}`
    : selectedFinalMarker
      ? `final-${selectedFinalMarker.lat}-${selectedFinalMarker.lng}-${selectedFinalMarker.categoryNames.join("|")}`
      : displaySub?.id
        ? `display-${displaySub.id}`
        : "default";

  if (!isLoaded || !isDataLoaded) {
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center text-indigo-400 font-bold text-2xl tracking-widest uppercase">
        Loading...
      </div>
    );
  }
  ...
  if (isDataLoaded && playersWithPaths.length === 0) {
    return (
      <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center text-white">
        <h2 className="text-2xl font-bold mb-4 text-indigo-400 tracking-widest uppercase">
          No Paths Found
        </h2>
        <p className="text-slate-400 mb-8">
          No one has left the house or recorded GPS data.
        </p>
        {isHost && (
          <button
            type="button"
            onClick={onFinishGame}
            className="px-6 py-3 bg-green-600 hover:bg-green-500 rounded-xl font-bold transition-all shadow-[0_0_20px_rgba(34,197,94,0.3)]"
          >
            End Game
          </button>
        )}
  ...
  const preloadMarkerPos =
    pathData?.rawPath.length > 0 ? pathData.rawPath[0] : dummyPos;

  const yesVotes = activeSubLatest
    ? Object.values(activeSubLatest.votes || {}).filter((v) => v === true)
        .length
    : 0;
  const noVotes = activeSubLatest
    ? Object.values(activeSubLatest.votes || {}).filter((v) => v === false)
        .length
    : 0;
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-slate-900">
      ...
      <div className="relative w-1/2 h-full z-10 flex-shrink-0">
        ...
        <div className="absolute top-6 left-6 right-6 z-10 flex justify-between items-start pointer-events-none">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-black uppercase text-indigo-400 drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]">
                Journey Replay
              </h1>
        ...
        {isLineComplete && !activeSubLatest && !isPreloading && (
          <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-40 bg-slate-800/95 backdrop-blur p-6 rounded-2xl border-2 border-indigo-500 shadow-[0_0_50px_rgba(79,70,229,0.4)] w-[350px] text-center animate-in zoom-in-90 duration-300">
            <h2 className="text-2xl font-black uppercase text-indigo-400 mb-2">
              {currentPlayer?.name}'s Journey
            </h2>
            ...
            {isHost ? (
              ...
              >
                {currentPlayerIndex < playersWithPaths.length - 1
                  ? "Next Player"
                  : "Show Podium"}
              </button>
            ) : (
      ...
      <div className="relative w-1/2 h-full bg-slate-800 z-20 shadow-[-20px_0_50px_rgba(0,0,0,0.5)] overflow-hidden">
        ...
        <div
          className={`absolute inset-0 pl-1.5 flex flex-col z-30 bg-slate-800 transition-all duration-500 ease-in-out ${isStreetViewVisible ? "opacity-100 translate-x-0 pointer-events-auto" : "opacity-0 translate-x-12 pointer-events-none"}`}
        >
          ...
          <div className="w-full bg-slate-900/95 backdrop-blur-xl border-t border-indigo-500/50 p-6 shadow-[0_-10px_40px_rgba(0,0,0,0.5)] z-20">
            {displaySub && !selectedSubmission && !selectedFinalMarker ? (
              <div className="max-w-xl mx-auto">
                <h3 className="text-2xl font-black text-white mb-1 text-center truncate">
                  {displaySub?.category}
                </h3>
                ...
                <div className="flex gap-4">
                  {(() => {
                    ...
                    const subPlayerTeam = players.find(
                      (p) => p.id === activeSubLatest?.player_id,
                    )?.team;
                    const myTeam = players.find((p) => p.id === playerId)?.team;
                    const isMySubmission =
                      playerId === activeSubLatest?.player_id;
                    const isMyTeamSubmission =
                      teamMode === "teams" &&
                      subPlayerTeam !== undefined &&
                      subPlayerTeam === myTeam;
                      ...
                      <>
                        <div className="flex-1 flex flex-col gap-2">
                          <button
                            ...
                            onClick={() =>
                              activeSubLatest &&
                              handleVote(activeSubLatest, true)
                            }
                            className={`w-full py-4 rounded-xl font-black uppercase text-lg border transition-all ${activeSubLatest?.votes?.[playerId] === true ? "bg-green-600 border-green-400 text-white shadow-[0_0_15px_rgba(34,197,94,0.5)]" : "bg-slate-800 border-slate-600 text-slate-300 hover:border-green-500 hover:text-green-500 hover:bg-green-900/30"}`}
                          >
                            Yes
                          </button>
                          <div className="text-center text-green-400 font-bold text-sm tracking-wide">
                            {yesVotes} Votes
                          </div>
                        </div>
                        <div className="flex-1 flex flex-col gap-2">
                          <button
                            ...
                            onClick={() =>
                              activeSubLatest &&
                              handleVote(activeSubLatest, false)
                            }
                            className={`w-full py-4 rounded-xl font-black uppercase text-lg border transition-all ${activeSubLatest?.votes?.[playerId] === false ? "bg-red-600 border-red-400 text-white shadow-[0_0_15px_rgba(239,68,68,0.5)]" : "bg-slate-800 border-slate-600 text-slate-300 hover:border-red-500 hover:text-red-500 hover:bg-red-900/30"}`}
                          >
                            No
                          </button>
                          <div className="text-center text-red-400 font-bold text-sm tracking-wide">
                            {noVotes} Votes
                          </div>
                        </div>
                      </>
              ...
              <div className="max-w-xl mx-auto">
                ...
                <p className="text-sm text-indigo-300 mb-4 text-center uppercase tracking-widest font-semibold">
                  Submission by{" "}
                  {
                    players.find((p) => p.id === selectedSubmission.player_id)
                      ?.name
                  }
                </p>
                <div className="mb-4 text-center">
                  ...
                  <div className="flex gap-4 justify-center">
                    <div className="text-green-400 font-bold">
                      Yes:{" "}
                      {
                        Object.values(selectedSubmission.votes || {}).filter(
                          (v) => v === true,
                        ).length
                      }
                    </div>
        ...
        <div
          className={`absolute inset-0 flex flex-col items-center justify-center p-8 z-20 transition-all duration-500 ease-in-out ${isStreetViewVisible ? "opacity-0 -translate-x-12 pointer-events-none" : "opacity-100 translate-x-0 pointer-events-auto"}`}
        >
          <div className="text-center mb-8">
            <h2 className="text-3xl font-black text-indigo-400 tracking-widest">
              {currentPlayer?.name}'s Board
            </h2>
          </div>

          {submissions.filter((s) => s.player_id === currentPlayer?.id)
            .length === 0 && (
          ...
          >
            {currentBoard?.map((category: string, idx: number) => {
              const sub = submissions.find(
                (s) =>
                  s.player_id === currentPlayer?.id && s.category === category,
              );
              ...
              let noPercent = 0;
              let tileClass =
                "bg-slate-900/60 border-slate-800 text-slate-600 opacity-60 [hyphens:auto] break-all";

              if (sub) {
                if (isReached) {
                  const yes = Object.values(sub.votes || {}).filter(
                    (v) => v === true,
                  ).length;
                  const no = Object.values(sub.votes || {}).filter(
                    (v) => v === false,
                  ).length;
                  const total = yes + no;
                  ...
                  tileClass =
                    "bg-slate-800 border-slate-600 text-white shadow-[inset_0_2px_10px_rgba(0,0,0,0.5)]";
                } else {
                  tileClass =
                    "bg-indigo-600/30 border-indigo-400 text-indigo-100 shadow-[0_0_20px_rgba(79,70,229,0.3)]";
                }
              }

              return (
                <div
                  key={`${currentPlayer?.id}-${idx}`}
                  className={`relative rounded-xl overflow-hidden flex items-center justify-center border-2 transition-colors duration-500 ${tileClass}`}
                >
                  <div
