...
export const generateNearbyStreetViewCategories = async (
  ...
  try {
    ...
    if (validImages.length < requiredCount) {
      throw new Error(
        `Nicht genug echte Straßen-Bilder in diesem Bereich gefunden (${validImages.length}/${requiredCount}). Bitte wähle einen größeren Radius.`,
      );
    }
    ...
    const prompt = getPromptForStreetViewCategories(
      validImages.length,
      difficulty,
      language,
    );
    ...
    if (!Array.isArray(parsedItems) || parsedItems.length < requiredCount) {
      throw new Error(
        `AI returned invalid format or fewer targets than required!`,
      );
    }
    ...
    const finalCategories: BingoCategory[] = parsedItems
      ...
      .filter(
        (item): item is BingoCategory & { score: number } => item !== null,
      )
      .filter(
        (cat, index, self) =>
          index ===
    ...
    if (finalCategories.length < requiredCount) {
      throw new Error(
        "Could not map all generated targets to images. Please try again.",
      );
    }
